...
// CẤU HÌNH TẠI ĐÂY
// ==========================================
const START_DATE_WEEK_1 = new Date("2026-06-15"); 
const GAPS = [{ afterWeek: 0, gap: 0 }];
...
function renderAll() {
    ...
    // 1. Tính toán ngày tháng và hiển thị tiêu đề
// Lấy dữ liệu từ CONFIG_APP (nếu không có thì để mặc định)
const hk = (typeof CONFIG_APP !== 'undefined') ? CONFIG_APP.hocKy : "2";
const nh = (typeof CONFIG_APP !== 'undefined') ? CONFIG_APP.namHoc : "2025-2026";

const pageTitle = document.querySelector('.page-title');
if (pageTitle) {
    // Kiểm tra nếu tiêu đề hiện tại có chữ "LỊCH THI" hoặc file có id/class đặc trưng cho thi
    const isExamWeek = pageTitle.innerText.includes("THI") || document.title.includes("Thi");
    
    if (isExamWeek) {
        pageTitle.innerHTML = `
            LỊCH THI KẾT THÚC HỌC PHẦN - HK${hk} (${nh}) <br>
            <span id="week-title" style="color: #b91c1c;">Tuần ${CURRENT_WEEK}</span> <br>
            <span class="subtitle">(Áp dụng từ ngày ${formatFull(mondayDate)} đến ${formatFull(sundayDate)})</span>
        `;
    } else {
        pageTitle.innerHTML = `
            Thời khóa biểu - HỌC KÌ ${hk} - NĂM HỌC: ${nh} (Tuần ${CURRENT_WEEK}) <br>
            <span class="subtitle">(Áp dụng từ ngày ${formatFull(mondayDate)} đến ${formatFull(sundayDate)})</span>
        `;
    }
}

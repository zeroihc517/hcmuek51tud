function loadTKBView() {
    ...
    $.ajax({
        url: SCRIPT_URL + "?action=getDeadlinesUser&mssv=" + currentUser.mssv,
        method: "GET", dataType: "json",
        success: function(data) {
            ...
            // Phải gọi render sau khi nạp data xong
            renderDeadlines(); 
        },
        error: function() {
...
function renderDeadlines() {
    ...
    let filtered = globalDeadlineData.filter(d => {
        ...
        // Nếu không có ngày bắt đầu/kết thúc (VLE hệ thống đôi khi trống), mặc định cho hiển thị hoặc lọc theo logic của bạn. 
        // Ở đây tạm cho hiện nếu ko có ngày cụ thể, còn có ngày thì phải lọt thỏm trong tuần
    ...
    container.innerHTML = filtered.map(item => {
        // Nút bấm: Ẩn nút sửa/xóa nếu là bài VLE kéo từ hệ thống
            ...
            if (combinedData.length === 0) {
                ...
                for (let baseName in groupedData) {
                    ...
                    groupItems.forEach((c, index) => {
                        ...
                        else if (c.ngayBatDau) { dateDisplay = c.ngayBatDau; } else if (c.ngayKetThuc) { dateDisplay = c.ngayKetThuc; }
...
function processTKBData(data) {
    globalTkbData = data.map((row) => {
        let lastElement = row.pop(); // Lấy phần tử cuối cùng ("SYS_..." hoặc số dòng)
        let actualRowIndex = -1;
        let isSystemFlag = false;

        // Kiểm tra xem ID có phải là định dạng của môn hệ thống không
        if (typeof lastElement === 'string' && lastElement.startsWith('SYS_')) {
            isSystemFlag = true;
            actualRowIndex = lastElement; // Giữ nguyên chuỗi "SYS_..." làm ID nhận diện
        } else {
    ...
    filterAndRenderTKB();
        ...
        function openEditTkbModal(sheetRowIndex) {
            ...
            // XỬ LÝ KHÓA CÁC TRƯỜNG CHO HỌC PHẦN HỆ THỐNG
            if (course.isSystem) {
...
function fetchSemesterConfig() {
 ...
 // --- HỆ THỐNG ĐÀO TẠO ---
    ...
    function buildSystemFilters() {
        // Lấy danh sách trực tiếp từ Cấu hình Học kỳ (chắc chắn khớp 100% với giao diện bên ngoài)
    ...
    // 2. Hàm mở Modal và TỰ ĐỘNG ĐỒNG BỘ
